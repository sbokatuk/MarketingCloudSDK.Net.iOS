...
# Builds the device test app against the packed SFMCSDK.Net packages, installs it on an iOS
...
PROJECT="${REPO_ROOT}/tests/MarketingCloudSDK.Net.iOS.DeviceTests/MarketingCloudSDK.Net.iOS.DeviceTests.csproj"
...
# version repeatedly and test yesterday's bits without this. Every package in the manifest is
# cleared, not just the ones referenced directly, so transitive arrivals cannot go stale either.
...
rm -rf "${REPO_ROOT}/tests/MarketingCloudSDK.Net.iOS.DeviceTests/obj" \
       "${REPO_ROOT}/tests/MarketingCloudSDK.Net.iOS.DeviceTests/bin"
...
APP_PATH="$(find "${REPO_ROOT}/tests/MarketingCloudSDK.Net.iOS.DeviceTests/bin/Release/${TARGET_FRAMEWORK}/${SIMULATOR_RID}" \
...
if ! grep -q "SFMC_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    xcrun simctl spawn "${udid}" log show --last 2m --predicate "process == 'MarketingCloudSDK.Net.iOS.DeviceTests'" \
        2>/dev/null | tail -100 | tee -a "${LOG_FILE}" || true
    echo "::error::SFMC simulator smoke tests failed or timed out"
    exit 1
fi
