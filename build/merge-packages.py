...
The two are tracked independently on purpose: a dependency-only meta-package (the pattern the sibling repositories use) is
...
consumer needs none of them - so such a package would restore on net10 without its siblings,
and the app would fail at link time with undefined Swift symbols.
